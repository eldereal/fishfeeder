...
module simple_gear(n, p = 0, center=[0,0,0], dir=1,
	circular_pitch=180,
	backlash=backlash,
	thickness=5) {
	translate(center) {
...
module planet(t, t_sun, n=3) {
	t_ring=t_sun+2*t_planet;
	r=(t+t_sun)/2;
	for (i=[1:n]) {
		rotate([0, 0, 360*i/n]) {
			simple_gear(n=t,p=t+1, center=[r,0,0], dir=-1);
			// translate([0, -2, 5]) {
			// 	color([0.5,0.5,0,5])
			// 	cube(size=[40, 4, 2]);	
			// }
...
t_ring=84;
t_sun=30;
t_planet=(t_ring-t_sun)/2;
t_carrier=t_sun*2+2*t_planet;

// sun(t=t_sun);
// planet(t=t_planet, t_sun=t_sun);
// scale([1, 1, 2])
// translate([0, 0, -5]){
	// difference(){
	// 	cylinder(r=t_sun/2+t_planet+5, h=5);
		translate([0, 0, -1]) {
			simple_gear(n=t_sun+2*t_planet, 
			p=0,
			dir=-1,backlash=0,thickness=7);	
		}		
	// }
// }

t_sun2=28;
t_planet2=(t_sun+2*t_planet-t_sun2)/2;
t_carrier2=t_sun2*2+2*t_planet2;

echo ("Planet 2 Teeth:", t_planet2);


// translate([0, 0, 5]) {
// 	rotate([0, 0, 180/t_ring])
	
// 	color([64/255, 64/255, 128/255]) {
// 		sun(t=t_sun2);
// 		planet(t=t_planet2, t_sun=t_sun2, n=4);	
// 	}
	
// }


echo ("Diameter", t_sun+2*t_planet+10);
echo ("Carrier Teeth:", t_carrier);
echo ("Carrier Speed ratio:", t_sun/t_carrier);
echo ("Carrier2 Teeth:", t_carrier2);
echo ("Carrier2 Speed ratio:", t_sun2/t_carrier2);
echo ("Diff ratio:",  t_sun/t_carrier-t_sun2/t_carrier2);
//planet
